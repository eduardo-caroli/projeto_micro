...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (parseLine(raw)) {
      Serial.println("\n--- Linha recebida ---");
      criaSeparados();
      criaDuplasLinhas(PINOS_ESPACAMENTO_SOLENOIDE);

      for (int v = 0; v < MAX_VETORES; v++) {
        int totalDuplas = totalDuplasGeradas[v];

        for (int i = 0; i < totalDuplas; i++) {

          int destino_x = pos_x - ESPACAMENTO_PONTOS;
          int destino_y = pos_y;

          Serial.print("Movendo para X:");
          Serial.print(destino_x);
          Serial.print(" Y:");
          Serial.print(destino_y);
          moverPara(destino_x, destino_y);
          marcaPontos(duplas[v][i][0],duplas[v][i][1]);
          Serial.println("");
        }

        moverPara(0, pos_y - ESPACAMENTO_PONTOS);
        pos_x = 0;
      }

      Serial.print("Linha Finalizada");
      moverPara(0, pos_y - ESPACAMENTO_LINHAS + ESPACAMENTO_PONTOS);
    }
  }
}
